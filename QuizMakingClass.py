     ...
     #---------Output dirs for logs--------------------------------------------------------------------------
           ...
           #----------------------------------------------------------------------------------------------
            ...
            while (not self.finish):
              ...
              #####  cv2.imshow("",quiz_image);cv2.waitKey()
...
if __name__ == '__main__':
    ...
    quiz_maker=make_quize(main_dir = input_dir,max_img_per_instance=1,max_img_total=100)
    quiz_maker.run_test(answer_question,ouput_dir=error_dir,save_error=True,save_all=True,queries=["Which panel contain identical (B,C,D) shape to the one in panel A, press letter to choose"])
